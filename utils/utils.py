...
data_dir = dict(
    ND0=os.getcwd() + '/filelists/ND0/',
    ND1=os.getcwd() + '/filelists/ND1/',
    ND2=os.getcwd() + '/filelists/ND2/',
    ND3=os.getcwd() + '/filelists/ND3/',
)
...
def get_image_size(model_name, dataset):
    if 'Conv' in model_name or model_name == 'ResNet12':
        image_size = 84
    else:
        image_size = 224
    return image_size
...
def get_resume_file(checkpoint_dir, epoch=None):
    ...
    filelist = [
        x for x in filelist
        if os.path.basename(x) != 'best_model.tar'
    ]
...
def get_checkpoint_dir(
    ...
    if addition is None:
        checkpoint_dir = base_path + '/save/checkpoints/%s/%s_%s' % (
            dataset,
            model_name,
            algorithm
        )
    else:
        checkpoint_dir = base_path + '/save/checkpoints/%s/%s_%s_%s' % (
            dataset,
            model_name,
            algorithm,
            str(addition)
    ...
    if algorithm not in ['baseline', 'baseline++']:
...
def one_hot(y, num_class):
    return torch.zeros((len(y), num_class)).scatter_(
        1,
        y.unsqueeze(1).long(),
        1
    )
