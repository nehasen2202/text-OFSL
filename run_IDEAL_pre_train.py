...
def pre_train():
    ...
    for epoch in range(start_epoch, pre_epoch):
        model.train()
        model.train_loop(epoch, pre_base_loader, optimizer)  # Train using pre-base loader
...
def ssl_train():
    ...
    for epoch in range(start_epoch, ssl_epoch):
        model.train()
        # Pass image_data to train_loop
        model.train_loop(epoch, ssl_base_loader, optimizer, sl_model=sl_model)
        if epoch == ssl_epoch - 1:
            outfile = os.path.join(ssl_dir, '{:d}.tar'.format(epoch))
            torch.save({'epoch': epoch, 'state': model.state_dict()}, outfile)
        model.eval()
        acc = model.test_loop(ssl_val_loader)
        if acc > max_acc:  # Save the best model
