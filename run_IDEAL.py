...
import warnings
warnings.filterwarnings("ignore", message="Producer process has been terminated before all shared CUDA tensors released")
...
def meta_train():
    ...
    model = IDEAL(model_func=model_dict[model_name], n_way=train_n_way, n_support=n_shot, image_size=image_size,
                  outlier=outlier, image_files=train_files, image_labels=train_labels, allc_files=allc_files, allc_labels=allc_labels ,noise_type=noise_type,
    ...
    for epoch in range(start_epoch, meta_epoch):
        ...
        if noise_type == 'OOT':
            acc = model.test_loop(meta_val_loader,
                                  image_files=allc_files,
                                  image_labels=allc_labels)
        else:
...
if __name__ == '__main__':
    ...
    if noise_type == 'OOT':
    # use a split that contains classes DIFFERENT from the test episode
    # 'val' works well; you could also use 'base'
       outlier_file = get_novel_file(dataset=dataset, split='allc')
    ...
    allc_labels = json.load(open(outlier_file))['image_labels']
    allc_files = json.load(open(outlier_file))['image_names']
