...
class NoiseMetaTemplate(nn.Module):
    def __init__(self, model_func, n_way, n_support, outlier=1, verbose=False, image_size=None, image_files=None,
                 image_labels=None, allc_files=None, allc_labels=None ,noise_type='IT', noise_rate=0.5, device='cuda:0'):
        ...
        # Store allc sets
        self.allc_files = np.array(allc_files) if allc_files else np.array([])
        self.allc_labels = np.array(allc_labels) if allc_labels else np.array([])
    ...
    def add_noise(self, x, y, noise_type, aug, return_y=False):
        ...
        if noise_type == 'IT':
            ...
            use_files = self.allc_files
            use_labels = self.allc_labels
            
            label_mask = np.ones_like(use_labels, dtype=int)
            for yy in y:
                label_mask[use_labels == yy] = 0
            images_range = np.arange(len(use_files))
            files = images_range[label_mask == 1]
            for ii in range(x.shape[0]):
                for jj in range(self.outlier):
                    file1 = np.random.choice(files)
                    y_new[ii, noise_idxes[ii, jj]] = file1
                    xx = Image.open(use_files[file1]).convert('RGB')
                    if aug:
