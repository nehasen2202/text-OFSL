...
class IDEAL(NoiseMetaTemplate):
    def __init__(self, model_func, n_way, n_support, image_size, outlier, ssl_feature_extractor,
                 image_files=None, image_labels=None, allc_files=None, allc_labels=None ,text_vectors=None,
        ...
        super(IDEAL, self).__init__(model_func,
                                    ...
                                    allc_files=allc_files,       # pass to parent
        ...
        self.combined_projection = nn.LazyLinear(visual_dim).to(self.device)
    ...
    def get_attention_class(self, x):
        ...
        z_support_norm = F.normalize(z_support, p=2, dim=2, eps=1e-12)
    ...
    def get_attention_support(self, x):
        ...
        attention_support = torch.softmax(sim, dim=1)  # [N*S, N]
    ...
    def train_loop(self, epoch, train_loader, optimizer, image_files=None, image_labels=None):
        ...
        for i, (x, y, text_vector) in enumerate(train_loader):
            ...
            torch.nn.utils.clip_grad_norm_(self.parameters(), 5.0) #added coz conv4+transformer was throwing an error
            optimizer.step()
    ...
    def test_loop(self, test_loader, return_std=False, image_files=None, image_labels=None):
        ...
        if self.noise_type == 'OOT':
            self.allc_files  = np.array(image_files)
            self.allc_labels = np.array(image_labels)
            
            
        acc_all = []
        iter_num = len(test_loader)  # Total number of iterations
        
    # NEW — make sure the OOT pool is available
