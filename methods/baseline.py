...
class Baseline(MetaTemplate):
    ...
    def train_loop(self, epoch, train_loader, optimizer):
        ...
        print_freq = 10
        avg_loss = 0
        for i, (x, y, *_) in enumerate(train_loader):  # Ignore text_vector
            x = x.to(self.device)
            y = y.to(self.device)

            optimizer.zero_grad()
            loss = self.forward_loss(x, y)  # `text_vector` is ignored here
            loss.backward()
            optimizer.step()
            avg_loss += loss.item()
    ...
    def test_loop(self, test_loader, return_std=False):
        ...
        with torch.no_grad():
            for i, (x, _, *_) in enumerate(test_loader):  # Ignore text_vector
                x = x.to(self.device)

                self.n_query = x.size(1) - self.n_support  # x:[N, S+Q, n_channel, h, w]
                self.n_way = x.size(0)

                scores = self.set_forward(x)  # `text_vector` is ignored here
